...
function run_tests {
  ...
  if [ $coverage -eq 1 ]; then
     testopts="$testopts --with-coverage --cover-erase --cover-package=nfvo "
  fi
...
function run_flake8 {
  ...
  pep8 --max-line-length=120 nfvo >pep8.log || true
  echo -e "$PREFIX Running pyflakes ..."
  pyflakes nfvo >pyflakes.log || true
...
if [ $update -eq 1 ]; then
  echo -e "$PREFIX Updating dependencies at test-requirements.txt":w
